...
void citirecorecta(char s[],bool &errorf)
{
    if(s[0]==0)
    {
        errorf=1;
        cout<<"NU ATI INTRODUS NICIO FUNCTIE";
    }
    if(strlen(s)>100)
    {
        errorf=1;
...
void altevariabile(char s[],bool &errorf)
{
...
void parantezare(char s[],bool &errorf)
{
    char v[10][10];
    int i;
    strcpy(v[0],"sin");
...
void scrierecorecta(char s[],bool &errorf)
{
    citirecorecta(s,errorf);
    parantezare(s,errorf);
    altevariabile(s,errorf);
    for(int i=0;i<strlen(s) && !errorf;i++)
    {
